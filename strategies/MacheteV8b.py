...
class MacheteV8b(IStrategy):
    ...
    custom_current_price_low_cache: TTLCache = TTLCache(maxsize=100, ttl=300)
    ...
    def get_entry_signal_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['MA_9'] = ta.MA(dataframe, timeperiod=9)
        dataframe['MA_10'] = ta.MA(dataframe, timeperiod=10)
        dataframe['MA_9_10'] = dataframe['MA_9'] - dataframe['MA_10']
    ...
    def get_current_price_low(self, pair: str, refresh: bool) -> float:
        if not refresh:
            rate_low = self.custom_current_price_low_cache.get(pair)
            # Check if cache has been invalidated
            if rate_low:
                return rate_low
        ask_strategy = self.config.get('ask_strategy', {})
        if ask_strategy.get('use_order_book', False):
            ob = self.dp.orderbook(pair, 1)
            rate_low = min([price[0] for price in ob[f"{ask_strategy['price_side']}s"]])
        else:
            ticker = self.dp.ticker(pair)
            rate_low = ticker['low']
        self.custom_current_price_low_cache[pair] = rate_low
        return rate_low
    ...
    def populate_trades(self, pair: str) -> dict:
        ...
        if self.config['runmode'].value in ('live', 'dry_run'):
            ...
            if active_trade:
                ...
                current_rate_low = self.get_current_price_low(pair, True)
                active_trade[0].adjust_min_max_rates(current_rate, current_rate_low)
